...
//    public static float? metersToByteRatio;
//    public static int? minHeightMetersAbs;
...
//        return result;
//    }
//    //private static byte[,] GetArea(byte[] values, int x, int y, int xl, int yl, int mapWidth)
//    //{
...
//    //        for (int hi = x, i = 0; hi < x + xl; hi++, i++)
//    //        {
//    //            result[i, j] = values[vi * mapWidth + hi];
//    //        }
//    //    }
//    //    return result;
//    //}
...
//            for (int hi = x, i = 0; hi < x + xl; hi++, i++)
//            {
//                result[j * xl + i] = values[vi * mapWidth + hi];
//            }
//        }
//        return result;
//    }
...
//            var pixelCount = file.Width * file.Height;
//            var pixels = new byte[pixelCount];
//            file.Mutate(n => n.Flip(FlipMode.Vertical));
//            file.CopyPixelDataTo(pixels);
...
//            List<byte[,]> heightAreas = [];
//            List<Vector2> areaCoordinates = [];
//            List<bool> coastAreas = [];
//            for (int vi = 0; vi < secondDerivative.GetLength(1); vi += samplesPerTile)
//            //for (int vi = secondDerivative.GetLength(1) - 1; vi >= 0; vi -= samplesPerTile)
//            {
//                for (int hi = 0; hi < secondDerivative.GetLength(0); hi += samplesPerTile)
//                {
//                    gradientAreas.Add(GetArea(secondDerivative, hi, vi, samplesPerTile, samplesPerTile));
...
//                    var areaBytes = GetArea(pixels, hi, vi, samplesPerTile, samplesPerTile, map.Settings.MapWidth);
//                    var unscaledHeights = areaBytes.Select(n => UnScaleHeight(map, n)).ToArray();
//                    if (unscaledHeights.Any(h => h < 0) && unscaledHeights.Any(h => h > 0))
//                    {

//                    }
//                    coastAreas.Add(unscaledHeights.Any(h => h < 0) && unscaledHeights.Any(h => h > 0));
//                }
//            }
...
//            var coastTiles = weightedDerivatives.Where(n => coastAreas[n.i]).ToArray();
//            var detail = new[]
//          {
//                coastTiles,
//                [],
//                [],
//                [],
//                weightedDerivatives.Except(coastTiles).ToArray(),
//            };

//            //var detail = new[]
//            //{
//            //    weightedDerivatives.Where(n => n.nonZeroP90 >= 0.005f).Union(weightedDerivatives.Where(n => coastAreas[n.i])).ToArray(),
//            //    weightedDerivatives.Where(n => n.nonZeroP90 is >= 0.001f and < 0.005f).ToArray(),
//            //    weightedDerivatives.Where(n => n.nonZeroP90 is >= 0.0005f and < 0.001f).ToArray(),
//            //    weightedDerivatives.Where(n => n.nonZeroP90 is >= 0.0001f and < 0.0005f).ToArray(),
//            //    weightedDerivatives.Where(n => n.nonZeroP90 < 0.0001f).ToArray(),
...
//    private static byte ScaleHeight(Map map, int meters)
//    {
//        minHeightMetersAbs ??= Math.Abs(map.MinHeight);
//        metersToByteRatio ??= (float)255 / (map.MaxHeight + minHeightMetersAbs);

//        return (byte)Math.Clamp((minHeightMetersAbs.Value + meters) * metersToByteRatio.Value, 0, 255);
//    }

//    private static float UnScaleHeight(Map map, byte byteHeight)
//    {
//        minHeightMetersAbs ??= Math.Abs(map.MinHeight);
//        metersToByteRatio ??= (float)255 / (map.MaxHeight + minHeightMetersAbs);
//        return byteHeight / metersToByteRatio.Value - minHeightMetersAbs.Value;
//    }
...
//            byte scaledHeight = ScaleHeight(map, h);
//            // byte scaledHeight = (byte)Math.Clamp((minHeightAbs + h) * totalHeightToByte, 0, 255);
