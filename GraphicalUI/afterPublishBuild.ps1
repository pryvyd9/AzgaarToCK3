...
dotnet publish -c Release -r win-x64 /p:PublishAot=true /p:PublishReadyToRun=true /p:IncludeNativeLibrariesForSelfExtract=true /p:DebugType=None /p:DebugSymbols=false /p:CopyOutputSymbolsToPublishDirectory=false --self-contained true -f net8.0-windows10.0.19041.0 -o "$($OutDir)win-x64"
dotnet publish -c Release -r maccatalyst-x64 /p:PublishSingleFile=true /p:PublishTrimmed=true /p:IncludeNativeLibrariesForSelfExtract=true /p:DebugType=None /p:DebugSymbols=false /p:CopyOutputSymbolsToPublishDirectory=false --self-contained true -f net8.0-maccatalyst -o "$($OutDir)osx-x64"
